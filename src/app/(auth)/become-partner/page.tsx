...
export default function BecomePartnerPage() {
    ...
    const init = async () => {
      try {
        ...
        try {
          ...
          // Ignore 404 if not registered yet
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.location.reload(); // Reload untuk menampilkan status pending
    } catch (error: any) {
      console.error(error);
  ...
  // TAMPILAN JIKA SUDAH MENDAFTAR (Status Pending / Rejected / Verified)
  if (providerData) {
    return (
          ...
          <div className="max-w-lg w-full bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
              ...
              <>
                ...
                <div className="mt-6 p-4 bg-gray-50 rounded-xl text-sm text-gray-600 text-left">
                  ...
                  <ul className="space-y-2">
                    ...
                    <li className="flex items-center gap-2"><CheckCircleIcon /> Foto Diri terupload</li>
            ...
            {providerData.verificationStatus === 'rejected' && (
              <>
                ...
                <button 
                  onClick={() => setProviderData(null)} // Reset state agar form muncul lagi
      ...
      <main className="flex-1 p-6 md:p-12 max-w-3xl mx-auto w-full">
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
          <div className="mb-8">
            ...
            <p className="text-gray-500">Lengkapi data diri dan dokumen pendukung untuk verifikasi.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider border-b pb-2">Informasi Keahlian</h3>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Deskripsi Keahlian & Bio</label>
                <textarea 
                  rows={4}
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-sm outline-none focus:border-red-500 resize-none"
                  placeholder="Ceritakan singkat tentang keahlian dan pengalaman kerja Anda..."
                  required
                />
              </div>
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-gray-400 mt-1">* Isi jika layanan membutuhkan mobilitas tinggi</p>
              </div>
            </div>

            {/* 2. UPLOAD DOKUMEN */}
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider border-b pb-2">Dokumen Verifikasi</h3>
                  ...
                  <div className="flex flex-col items-center">
                    {files.ktp ? (
                      ...
                      <>
                        ...
                        <span className="text-[10px] text-gray-400">Max 5MB, Jelas terbaca</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center hover:bg-gray-50 transition-colors relative">
                  ...
                  <div className="flex flex-col items-center">
                    {files.certificate ? (
                      ...
                      <>
                        <UploadIcon />
                        <span className="text-xs font-bold text-gray-500 mt-2">Sertifikat Keahlian (Opsional)</span>
            ...
            <button
              ...
              className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-red-100 transition-all flex items-center justify-center disabled:opacity-70 disabled:cursor-not-allowed"
            >
